...
> host it is a brand-new core. If you are upgrading from the old
> `jarvis_hud_core`, see [Upgrading from Jarvis HUD](#upgrading-from-jarvis-hud).
...
| **Mic Echo Cancellation / Noise Suppression / Auto Gain Control** | on / on / on | Browser DSP stages applied to this screen's **tap-to-talk mic capture** (v1.50.5). When transcripts come back mangled: try **Auto Gain Control** off first (AGC re-adapts each capture and can pump — early utterances fine, later ones gibberish), then Noise Suppression (eats phonemes), then Echo Cancellation (chews distant/quiet mics). Does **not** affect the wake-word listener — that engine opens its own mic with browser defaults. |
| **Mic Sample Rate (Hz)** | `0` (device default) | Sample rate requested for this screen's tap-to-talk capture (8000–96000). The audio is downsampled to 16 kHz for speech recognition either way; unsupported values fall back to the device's own. |
| **Mic Sample Size (bits)** | `0` (device default) | Bit width requested for the capture (8 / 16 / 32); unsupported values fall back to the device's own. |
...
- **Mic dial-in**: the capture constraints are per screen (Screens tab —
  Echo Cancellation, Noise Suppression, Auto Gain Control, Sample Rate,
  Sample Size; see the profile table above). For transcript trouble, change
  one DSP toggle at a time and retest a few rounds — AGC/NS re-adapt per
  capture, so quality can *degrade* over a session rather than fail
  consistently. The screen console logs the browser's actually-granted
  capture settings (`[screen] mic capture settings {…}` — device, rate,
  DSP chain) on every mic open; the core requests no `deviceId`, so Chrome
  uses its default microphone (`chrome://settings/content/microphone`).
...
| Tap-to-talk transcripts mangled / degrade after a few turns | Dial in the per-screen **Mic** fields (Screens tab): turn Auto Gain Control off first, then Noise Suppression, then Echo Cancellation — one at a time, retest several rounds. Check the console's `[screen] mic capture settings` log for what the browser actually granted, and confirm Chrome's default microphone (`chrome://settings/content/microphone`) is the mic you think it is — the core requests no specific device. |
